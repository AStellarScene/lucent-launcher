...
This copy is vendored by Lucent Launcher from the upstream `0.1.1` release.
Launcher-specific fixes and regression tests belong here so the application can
improve the backend without depending on an unpublished registry build. Keep
this crate GUI-free and preserve its public facade where practical.
...
## Use in Lucent Launcher

Lucent Launcher vendors this crate under `crates/mc-launcher-core` and uses a
path dependency so backend fixes can be tested with the application:

```toml
[dependencies]
mc-launcher-core = { path = "crates/mc-launcher-core" }
```

The upstream registry release remains the reference API for compatibility, but
changes made in this repository should be validated with:
...
`ProgressEvent`. Download plans report their task count, task lifecycle, and
byte counts when the server provides a content length. This allows callers to
render determinate progress for downloads while keeping metadata and external
installer phases indeterminate:
